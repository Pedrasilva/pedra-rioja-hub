...
/**
 * Stages a parsed statement for review. Fingerprints are recomputed server
 * side so the client cannot weaken duplicate detection, and rows already
 * present in the account are flagged (never silently dropped).
 */
export const stageStatementImport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return stageStatementImportCore(context.supabase, data);
  });
